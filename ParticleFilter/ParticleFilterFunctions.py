...
def main():
    global newDOGMA, new_covar_xy_vel, new_var_y_vel, new_var_x_vel
    ...
    Vb = 2 * 10 ** 4  # number of newborn particles
    ...
    start = time.time()
    particle_array = ParticleArray(V, grid_cell_array.get_shape(), state_size, T, p_S, scale_vel, scale_acc,
                                   process_pos, process_vel, process_acc)
    end = time.time()
    ...
    for i in range(N):
        ...
        if p_B == 0:
            ...
            empty_array = False
        birth_particle_array = ParticleArray(Vb, grid_cell_array.get_shape(), state_size, T, p_S, scale_vel, scale_acc,
                                             process_pos, process_vel, process_acc, birth=True, empty_array=empty_array)
        ...
        if (i + 1) > index_stopped:
            newDOGMA, new_var_x_vel, new_var_y_vel, new_covar_xy_vel = get_dogma(grid_cell_array, grids, state_size,
                                                                                 grids[i, :, :], shape)
        ...
        particle_array_next = ParticleArray(V, grid_cell_array.get_shape(), state_size, T, p_S,
                                            scale_vel, scale_acc, process_pos, process_vel, process_acc,
                                            empty_array=True)
        ...
        if do_plot:
            ...
            colorwheel_plot(head_grid, occ_grid=occ_grid, m_occ_grid=DOGMA[i][0, :, :],
                            title=os.path.join(OUTPUT_DIR, title), show=True, save=True)
